...
<p><br>If you read images that contain a clear gradient, you're mostly fine with the default settings.<br><br>If not, <b>ImageGradientExtractor</b> has a third parameter to set the specific line (Y coordinate) you want to scan while extracting the gradient (you may also set this later by calling <code>setScanPosition</code>).<br>By default, the middle line of the image is scanned, however you can set it to any positive integer (Y position on the image), or a constant defined by the class.<br>There is one constant that is particularly interesting, <code>SCAN_AVERAGE</code>, which resamples the image to 1px height (once) and returns the (average) color at the given X from this copy.<br><br>Let's see some examples with more sophisticated images, so you can clearly see the differences given by this setting:</p>
